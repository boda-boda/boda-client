              ...
              <S.InfoTable>
                <tbody>
                  <tr>
                    <td rowSpan={9} className="recipientProfile">
                      <S.ProfileImageContainer>
                        <S.ProfileImage src={careWorker.careWorker.profile} />
                      </S.ProfileImageContainer>
                    </td>
                    <th>이름</th>
                    <td className="left">{careWorker.careWorker.name}</td>
                    <th>성별</th>
                    <td className="right">{careWorker.careWorker.gender}</td>
                  </tr>
                  <tr>
                    <th>생년월일</th>
                    <td className="select left">
                      {careWorker.careWorker.birthDay} ({careWorker.careWorker.age})세
                    </td>
                    <th>돌봄 시간</th>
                    <td className="right">{careWorker.careWorker.availableTime}</td>
                  </tr>
                  <tr>
                    <th rowSpan={1}>주소</th>
                    <td colSpan={3}>
                      ({careWorker.careWorker.zipCode}) {careWorker.careWorker.address}{' '}
                      {careWorker.careWorker.detailAddress}
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td colSpan={1} className="">
                      <S.TdFlexBox>
                        <S.ToggleButton>
                          {careWorker.careWorker ? careWorker.careWorker.religion : ''}
                        </S.ToggleButton>
                      </S.TdFlexBox>
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td colSpan={3} className="overtd">
                      <S.TdFlexBox>
                        {careWorker.careWorkerCapabilities
                          ? careWorker.careWorkerCapabilities.map((capability, index) => {
                              return (
                                <S.ToggleButton
                                  className="overitems"
                                  key={`careInfoListItem-${index}`}
                  ...
                  <tr>
                    <th>메모</th>
                    <td colSpan={3} className="select wide">
                      {careWorker.careWorker.description}
                    </td>
                  </tr>
                </tbody>
              </S.InfoTable>
