...
export default function CareGiverListDemo() {
  const careCenter = useCareCenter();

  const [rerender, setRerender] = useState(false);

  // const [CARE_WORKERS_DEMO, setCareWorkers] = useState([] as CareWorker[]);
  ...
  const filterTime = (cws: any[]) => {
    const result =
      selectedTime === ''
        ? cws
        : cws.filter((cw: any) => cw.time === selectedTime || cw.time === '종일');
    return result;
  };

  const filterWorkingState = (cws: any[]) => {
    const result =
      selectedWorkingState.length === 0
        ? cws
        : cws.filter((cw: any) => selectedWorkingState.includes(cw.workingState));
    return result;
  };
  ...
  const handleSearchOnClickSearchButton = () => {
    ...
    setFilteredCareWorkers(
      filterNameByLetter(
        filterTime(
          filterWorkingState(
            filterReligion(filterCareInfo(filterArea(filterName(CARE_WORKERS_DEMO))))
          )
        )
      )
    );
